...
// TMC2660Stepper::TMC2660Stepper(spi_master_device_t *dev,
//                                spi_master_t *spi,
//                                uint32_t cs_pin,
//                                int cpol,
//                                int cpha,
...
//     TMCStepper(default_RS), // Assuming default_RS is a valid argument for TMCStepper's constructor
//     dev(dev), spi(spi), cs_pin(cs_pin) // Initializing member variables
// {
...
//     uint8_t dataToSend = (dummy >> 16) & 0xFF; // Extract the byte
//     spi_master_transfer(&dev, &dataToSend, &response, 8); // Pass the address of `receivedData` for `data_in`
//     response <<= 8;
//     dataToSend = (dummy >>  8) & 0xFF;
//     spi_master_transfer(&dev, &dataToSend, &response, 8); // Pass the address of `receivedData` for `data_in`
//     response <<= 8;
//     dataToSend = dummy & 0xFF;
//     spi_master_transfer(&dev, &dataToSend, &response, 8); // Pass the address of `receivedData` for `data_in`
...
// spi_master_start_transaction(*dev);
//     uint8_t dataToSend = (data >> 16) & 0xFF; // Extract the byte
//     spi_master_transfer(&dev, &dataToSend, &response, 8); // Pass the address of `receivedData` for `data_in`
//     dataToSend = (data >>  8) & 0xFF;
//     spi_master_transfer(&dev, &dataToSend, &response, 8); // Pass the address of `receivedData` for `data_in`
